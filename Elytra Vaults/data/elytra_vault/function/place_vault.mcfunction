...
# (lines beginning with $ are macro lines; $(key_item) / $(key_name) come from storage)
$setblock ^ ^-1 ^ vault{config:{loot_table:"elytra_vault:elytra_vault",key_item:{id:"$(key_item)"}},shared_data:{display_item:{id:"minecraft:elytra",count:1},loot_table:"elytra_vault:elytra_vault"}} replace
